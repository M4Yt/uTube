...
var vt = {
	...
	getMyContent: function(url, pattern, clean) {
		...
		if (clean) myPageContent = vt.cleanMyContent(myPageContent, true);
	...
	ytVideoList: function(url) {
		var ytVideosContent = vt.getMyContent(url, '"url_encoded_fmt_stream_map":\\s+"(.*?)"', false);
		if (ytVideosContent) ytVideosContent = vt.cleanMyContent(ytVideosContent, false);
		...
		if (ytVideosContent) {
			...
			for (var i = 0; i < ytVideos.length; i++) {
				...
				if (ytVideoCode) {
					myVideoCode = vt.FORMATS[ytVideoCode];
					if (myVideoCode) {
						...
						ytVideo = vt.cleanMyContent(ytVideo, true);
